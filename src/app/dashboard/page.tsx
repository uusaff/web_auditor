...
export default function UserDashboard() {
  ...
  return (
    <main className="min-h-screen w-full text-white font-['var(--font-montserrat)'] flex flex-col bg-transparent">
      {/* Dashboard Content */}
      <section className="flex-1 w-full max-w-7xl mx-auto px-6 py-12 flex flex-col">
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" aria-busy="true">
            {[1, 2, 3].map(i => (
              <div key={i} className="h-48 rounded-2xl bg-white/5 animate-pulse border border-white/10" aria-hidden="true"></div>
            ))}
          </div>
        ) : history.length === 0 ? (
          <section className="flex flex-col items-center justify-center p-12 rounded-2xl border border-sky-400/20 bg-[#101b13]/40 text-center mx-auto mt-8 w-full max-w-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {history.map((item) => (
              <Link 
                ...
                className="block"
              >
                <article className="group relative h-48 rounded-2xl bg-white/5 border border-white/10 overflow-hidden flex flex-col p-6 hover:bg-white/10 transition-all hover:border-white/20">
